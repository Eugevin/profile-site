...
onMounted(() => {
  const title = document.querySelector('.home__title') as any;
  title.innerHTML = title.textContent?.replace(/\S/g, "<span class='letter'>$&</span>");

  const suptitle = document.querySelector('.home__suptitle') as any;
  suptitle.innerHTML = suptitle.textContent.replace(/\S/g, "<span class='letter'>$&</span>");

  anime({
    targets: '.home__banner img',
    translateX: ['-110%', 0],
    scale: [1.1, 1],
    opacity: [0, 1],
  ...
  anime({
    ...
    easing: "easeOutExpo",
    duration: 1000,
    delay: (el: HTMLElement, i: number) => 1500 + 20 * i
  });
  anime({
    targets: '.home__suptitle .letter',
    translateY: [16, 0],
    opacity: [0, 1],
    easing: "easeOutExpo",
    duration: 1000,
